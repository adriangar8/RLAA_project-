"""
Experiment: Behavioral Cloning + PPO + Entropy Annealing

Pipeline
Expert → Demonstrations → BC → PPO (entropy annealing) → Evaluation
"""

import os
import gymnasium as gym
import torch
import numpy as np

from stable_baselines3 import PPO
from stable_baselines3.common.vec_env import DummyVecEnv

from src.policy import PolicyNetwork
from src.expert import DemonstrationDataset
from src.bc import BehavioralCloning, BCConfig
from src.evaluate import evaluate_policy

from src.entropy_annealing_callback import EntropyAnnealingCallback

# -------------------------------------------------------
# Config
# -------------------------------------------------------

ENV_ID = "LunarLander-v3"
DEMO_PATH = "data/demonstrations_200ep.npz"

PPO_TIMESTEPS = 500_000

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"


# -------------------------------------------------------
# Transfer BC weights → PPO
# -------------------------------------------------------

def transfer_bc_weights_to_ppo(bc_policy, ppo_model):

    bc_state = bc_policy.state_dict()
    ppo_state = ppo_model.policy.state_dict()

    matched = 0

    for ppo_key, ppo_val in ppo_state.items():
        for bc_key, bc_val in bc_state.items():

            if ppo_val.shape == bc_val.shape:
                ppo_state[ppo_key] = bc_val.detach().cpu()
                matched += 1
                break

    ppo_model.policy.load_state_dict(ppo_state)

    print(f"Transferred {matched} BC layers into PPO policy")


# -------------------------------------------------------
# 1 Load demonstrations
# -------------------------------------------------------

print("\nLoading demonstrations")

demo = DemonstrationDataset.load(DEMO_PATH)

obs_dim = demo.observations.shape[1]
act_dim = len(np.unique(demo.actions))


# -------------------------------------------------------
# 2 Train Behavioral Cloning
# -------------------------------------------------------

print("\nTraining BC policy")

policy = PolicyNetwork(obs_dim, act_dim)

bc_cfg = BCConfig()

bc_trainer = BehavioralCloning(policy, bc_cfg)

bc_history = bc_trainer.train(demo)


# -------------------------------------------------------
# 3 Evaluate BC
# -------------------------------------------------------

print("\nEvaluating BC")

bc_results = evaluate_policy(policy, ENV_ID)

print(
    f"BC performance: {bc_results['mean']:.2f} ± {bc_results['std']:.2f}"
)


# -------------------------------------------------------
# 4 Initialize PPO
# -------------------------------------------------------

print("\nInitializing PPO")

env = DummyVecEnv([lambda: gym.make(ENV_ID)])

ppo_model = PPO(
    "MlpPolicy",
    env,
    learning_rate=3e-4,
    n_steps=1024,
    batch_size=64,
    n_epochs=10,
    gamma=0.999,
    gae_lambda=0.98,
    ent_coef=0.01,   # initial entropy
    verbose=1,
)


# -------------------------------------------------------
# 5 Transfer BC weights
# -------------------------------------------------------

transfer_bc_weights_to_ppo(policy, ppo_model)


# -------------------------------------------------------
# 6 Entropy annealing callback
# -------------------------------------------------------

entropy_callback = EntropyAnnealingCallback(
    start_ent=0.02,
    end_ent=0.001,
    verbose=1
)


# -------------------------------------------------------
# 7 PPO Fine-tuning
# -------------------------------------------------------

print("\nStarting PPO fine-tuning with entropy annealing")

ppo_model.learn(
    total_timesteps=PPO_TIMESTEPS,
    callback=entropy_callback
)


# -------------------------------------------------------
# 8 PPO wrapper for evaluation
# -------------------------------------------------------

class PPOPolicyWrapper:

    def __init__(self, model):
        self.model = model

    def act(self, obs, deterministic=True):
        action, _ = self.model.predict(obs, deterministic=deterministic)
        return int(action)


ppo_policy = PPOPolicyWrapper(ppo_model)


# -------------------------------------------------------
# 9 Evaluate BC + PPO + Annealing
# -------------------------------------------------------

print("\nEvaluating BC + PPO + Entropy Annealing")

ppo_results = evaluate_policy(ppo_policy, ENV_ID)

print(
    f"PPO + Annealing performance: {ppo_results['mean']:.2f} ± {ppo_results['std']:.2f}"
)


# -------------------------------------------------------
# 10 Save results
# -------------------------------------------------------

os.makedirs("results", exist_ok=True)

np.save(
    "results/bc_ppo_annealing.npy",
    {
        "bc": bc_results,
        "ppo_annealing": ppo_results
    }
)

print("\nSaved results → results/bc_ppo_annealing.npy")

